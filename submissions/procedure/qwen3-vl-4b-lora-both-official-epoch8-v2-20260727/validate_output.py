...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument(
        "--require-nonempty",
        action="store_true",
        help="fail when a response has empty/whitespace-only content (smoke tests)",
    )
...
def main() -> int:
    ...
    for index, row in enumerate(answers):
        ...
        elif args.require_nonempty and not row["content"].strip():
            errors.append(f"response {index} has empty content")
