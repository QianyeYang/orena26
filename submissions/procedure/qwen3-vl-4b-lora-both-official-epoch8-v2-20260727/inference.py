...
WINDOW_SYSTEM_PROMPT = (
    "You are an expert surgical vision assistant analysing a time window from a "
    "surgical procedure. The window is shown as frames in chronological order, "
    "each labelled with its absolute timestamp (hh:mm:ss from the start of the "
    "original procedure video). Timestamps in the question refer to that same "
    "absolute timeline. Look carefully at the frames and answer the question. "
    "Respond with ONLY the answer in the exact requested format — no explanation, "
    "no extra words."
)
...
def load_fo_definitions(path: Path) -> tuple[str, tuple[str, ...]]:
    """Load challenge-provided definitions and recover all class headings.

    The public file is a JSON-encoded Markdown-like string whose class headings
    are followed by a line of dashes. Test-time metadata may add classes, so the
    packaged core list is only a fallback and never an upper bound.
    """
    value = json.loads(path.read_text(encoding="utf-8"))
    if not isinstance(value, str) or not value.strip():
        raise TypeError("FO_definitions.json must contain a non-empty JSON string")

    discovered: list[str] = []
    lines = value.splitlines()
    for index in range(len(lines) - 1):
        name = " ".join(lines[index].split())
        underline = lines[index + 1].strip()
        if name and len(name) <= 100 and re.fullmatch(r"-{3,}", underline):
            discovered.append(name)

    names: list[str] = []
    seen: set[str] = set()
    for name in (*CORE_FO_NAMES, *discovered):
        key = name.casefold()
        if key not in seen:
            names.append(name)
            seen.add(key)
    return value.strip(), tuple(names)
...
def infer_answer_format(question: str) -> str:
    ...
    if re.search(
        r"\b(?:please )?select "
        r"(?:one answer|(?:none, )?one or multiple answers?)\s*:",
        text,
    ):
        return "multiple_choice"
    if re.search(
        r"(?:please )?answer with (?:exactly )?yes or no[.!]?$",
        text,
    ):
        return "binary"
    if "format xx%" in text or "answer as a percentage" in text:
        return "percentage"
    if "hh:mm:ss" in text:
        return "time"
    if re.search(r"\bplease provide (?:a )?number\b", text):
        return "number"
    if (
...
def build_instruction(
    ...
    """Build a strict output instruction, including runtime-defined FO classes."""
...
def normalize_binary(text: str) -> str:
    ...
    if lower in {"yes", "no"}:
        return lower
    leading = re.match(r"^(yes|no)\b", lower)
    if leading:
        return leading.group(1)
    if re.search(
        r"\b(no|nope|false|incorrect|absent|negative|not present)\b", lower
    ):
        return "no"
    if re.search(
...
def normalize_fo_class(text: str, fo_names: tuple[str, ...]) -> str:
    lower = clean_text(text).lower()
    if not lower:
        return "none"
...
def normalize_answer(
    ...
    *,
    ...
    if answer_format == "time":
        matches = re.findall(r"(\d{1,2}):([0-5]?\d):([0-5]?\d)", raw)
        if matches:
            return ", ".join(
...
def build_messages(
    ...
    if request.start_time <= 0.5:
        span = f"The surgical video from its start (00:00:00) up to {end_timestamp}"
    else:
        start_timestamp = seconds_to_timestamp(request.start_time)
        span = (
            f"This procedure clip covers absolute video time {start_timestamp} "
            f"to {end_timestamp}"
        )

    if len(indices) > 1:
        preamble = (
            f"{span} and is shown as {len(indices)} frames in chronological order, about "
            f"{interval:.0f} s apart, each labelled with its absolute video "
            "timestamp."
        )
    else:
        preamble = (
            f"{span} and is shown as a single frame labelled with its absolute "
            "video timestamp."
        )
    ...
    content: list[dict[str, str]] = [
        {"type": "text", "text": preamble},
        {
            "type": "text",
            "text": (
                "Challenge-provided foreign-object definitions:\n"
                f"{fo_definitions}"
            ),
        },
    ]
...
def run() -> int:
    ...
    fo_definitions, fo_names = load_fo_definitions(definitions_path)
    ...
    log.info(
        "loaded %d challenge foreign-object definitions (%d chars)",
        len(fo_names),
        len(fo_definitions),
    )
