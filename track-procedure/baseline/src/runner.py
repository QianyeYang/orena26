#!/usr/bin/env python
"""PROCEDURE zero-shot baseline: video prefix -> timestamped frames -> VLM -> rich rows.

Thin CLI over the shared ``src.videovqa.run_video_qa`` loop with PROCEDURE
defaults: 10-second extraction grid (stride 250 @ 25 fps), <= 128
evenly-subsampled frames per prefix ``[00:00:00, T]`` (T up to ~5 h),
``max_pixels`` 262144 (960x540 -> 672x384 -> 252 tokens/frame).
...
def main() -> None:
    ...
    ap.add_argument("--stride", type=int, default=250, help="grid: every Nth source frame")
    ap.add_argument("--max-frames", type=int, default=128, help="frames per prefix after subsample")
